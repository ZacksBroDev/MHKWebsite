...
// Connect to MongoDB
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/mhk_karate', {
      ...
      fs.createReadStream(csvPath)
        ...
        .on('data', (row) => {
          ...
          const event = {
            ...
            maxParticipants: row.type === 'Tournament' ? 50 : row.type === 'Class' ? 20 : 30,
        ...
        .on('end', async () => {
          try {
            ...
            console.log(`Successfully loaded ${result.length} events into MongoDB`);
            
            // Display some sample events
            console.log('\nSample events loaded:');
            result.slice(0, 5).forEach(event => {
              console.log(`- ${event.date.toISOString().split('T')[0]}: ${event.title} at ${event.time}`);
            });
